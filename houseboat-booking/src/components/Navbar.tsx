...
export function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${isScrolled ? 'bg-white shadow-sm' : 'bg-transparent'}`}>
        ...
        <div className={`flex items-center gap-2 md:gap-4 transition-all duration-500 ${isScrolled ? 'h-16' : 'h-20'}`}>
          ...
          <Link href="/" className="flex flex-col items-start gap-1 group shrink-0 pt-1">
            <div className="flex items-center gap-2">
              ...
              <span className={`text-xl md:text-2xl font-semibold tracking-tight ${isScrolled ? 'text-ocean-blue' : 'text-white'}`}>
                Kerala<span className="text-gold">Houseboats</span>
              </span>
            </div>
            <span className={`text-[8px] sm:text-[10px] md:text-xs font-medium tracking-wide ${isScrolled ? 'text-gray-500' : 'text-white/90'}`}>
          ...
          <div className="flex-1 flex items-center justify-center min-w-0">
            ...
            <div className="hidden lg:flex items-center gap-1">
              <Link href="/" className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${isScrolled ? 'text-ocean-blue hover:bg-ocean-blue/10' : 'text-gray-100 hover:text-white hover:bg-white/10'}`}>
                Home
              </Link>
              
              <Link href="/houseboats" className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${isScrolled ? 'text-ocean-blue hover:bg-ocean-blue/10' : 'text-gray-100 hover:text-white hover:bg-white/10'}`}>
                Houseboats
              </Link>
              ...
              <Link href="/contact" className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${isScrolled ? 'text-ocean-blue hover:bg-ocean-blue/10' : 'text-gray-100 hover:text-white hover:bg-white/10'}`}>
                Contact
              </Link>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            {/* Mobile Menu Toggle */}
            <button 
              className="lg:hidden flex items-center justify-center w-10 h-10 bg-white hover:bg-gray-50 border border-gray-200 rounded-full transition-all shadow-sm shrink-0"
      ...
      {/* Mobile Nav Content */}
      {isMobileMenuOpen && (
        <div className="lg:hidden absolute top-full left-0 right-0 bg-white shadow-lg py-4 px-4 flex flex-col gap-4 text-gray-800 border-t border-gray-100">
          <Link href="/" onClick={() => setIsMobileMenuOpen(false)} className="px-4 py-2 hover:bg-gray-50 rounded-lg">Home</Link>
